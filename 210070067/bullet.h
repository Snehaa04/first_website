...
public:
    ...
    {
        // move the bullet
        double new_y = rect.getY() - vy*t;  //decreasing y coordinate
        if(new_y < (0-rect.getHeight()/2.0))   // out of canvas, so delete it(code in bubbles.h file)
        {
            return false;
        }
        rect.moveTo(rect.getX(), new_y);//  same x coordinate only change in the y coordinate
        return true;
    }
